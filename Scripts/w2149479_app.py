...
plotfig = px.line(trend, x='year',y='new_displacement',markers=True)
...
no_of_hazard = df.groupby('hazard_type_name')['new_displacement'].sum().reset_index().sort_values('new_displacement', ascending=False)
...
top_10 = df.groupby('country_name')['new_displacement'].sum().reset_index().nlargest(10,'new_displacement')
topfig = px.bar(top_10, x='country_name', y='new_displacement', color ='new_displacement', color_continuous_scale='Reds')
...
map = df.groupby('country_name')['new_displacement'].sum().reset_index()
mapfig = px.choropleth(map,locations='country_name', locationmode='country names', color='new_displacement',hover_name='country_name',color_continuous_scale=px.colors.sequential.Plasma)
